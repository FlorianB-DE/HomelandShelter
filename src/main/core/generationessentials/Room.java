...
public class Room extends Point {
	...
	public Room(int size, int x, int y) throws
			RoomGenerationObstructedException {
		this(size, size, x, y);
	}

	public Room(int sizeX, int sizeY, int x, int y) throws
			RoomGenerationObstructedException {
		super(x, y);
		this.sizeX = sizeX;
		this.sizeY = sizeY;
	 ...
	 *             'doors' List if there are no more than one door already in
	 *             the
	 *             List. this prevents the Room to have to many Doors,
	 *             therefore
	...
	protected void generateRoom() throws RoomGenerationObstructedException {
		// looping from negative half size to positive half size ensures that
		// the x and
		...
		for (int i = -sizeX / 2; i <= sizeX / 2; i++) {
			...
			{
				if (x + i < DungeonGenerator.SIZE &&
					y + j < DungeonGenerator.SIZE && x + i >= 0 && y + j >= 0) {

					// only generates a room if the array is empty at the
					// specified coordinates
					if (DungeonGenerator.getTileAt(x + i, y + j) == null) {

						// occupies a tile
						DungeonGenerator.setTileAt(x + i, y + j,
												   new RoomFloor(x + i,
																 y + j));

						// if the current loop is at a border of a room this
						// set of conditions try to
						// generate a door

						// true when iterator is at the most left edge without
						// the most upper/lower case
						...
						// true when iterator is at the most right edge
						// without the most upper/lower
						// case
						...
						// true when iterator is at the most upper edge
						// without the most left/right case
						else if (j == -sizeY / 2 && !(i == -sizeX / 2) &&
								 !(i == sizeX / 2)) {
							try {
						...
						// true when iterator is at the most lower edge
						// without the most left/right case
						else if (j == sizeY / 2 && !(i == -sizeX / 2) &&
								 !(i == sizeX / 2)) {
							try {
						...
						// true when both iterators are on their peak
						// value. Checks weather the room has
						// a door or not.
						// If it doesn't, a door will be created by force.
						// If it fails, it cancels the
	...
	/**
	 * @param i the current state of the first dimensions iterator from
	 *             where to
	 *          decompose
	 * @param j the current state of the second dimensions iterator from
	 *             where to
	 *          decompose sets every tile from i, j to negative half the sizes
	 *          (both
	...
	/**
	 * @return the first instance of Door in 'doors' List used by the
	 * PathFinder to
	 * path TO
