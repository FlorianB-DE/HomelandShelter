...
public abstract class DungeonGenerator {
	...
	public static Tile[][] generateDungeon() {
		...
		rooms = new Room[roomCount + (int) Math
				.round(Math.random() * deviation - deviation / 2)];
		...
		for (int i = 0; i < values.length; i++) {
			for (int j = 0; j < values[i].length; j++) {
				...
				// if a values is higher than 0.5 a room generation will be
				// initiated
				if (values[i][j] >= 0.5) {
					...
					// determines the size of the room by iterating the
					// 'values' array until the
					...
					for (int k = 0; k < rooms.length; k++) {
						...
						// if it finds a free spot in the 'rooms' array it
						// will try to generate a room
						if (rooms[k] == null) {
							try {
								// room generation
								rooms[k] = new Room(room_sizeX, room_sizeY,
													i - room_sizeX / 2,
													j - room_sizeY / 2);
		...
		for (int i = 0; i < rooms.length - 1; i++) {
			...
			while (it.hasNext()) {
				...
				if (!current.equals(rooms[i + 1].getEntrance().getLocation())) {
					if (it.hasPrevious()) {
						try {
							if (!current.equals(paths.get(
									it.previousIndex() - 1)) &&
								!current.equals(paths.get(it.nextIndex())) &&
								!paths.get(it.nextIndex()).equals(paths.get(
										it.previousIndex() - 1))) {
								if (NeighbourFinder
											.pathableNeighbours(current.x,
																current.y,
																tiles) <= 2) {
									if (paths.get(it.previousIndex() - 1).x ==
										paths.get(it.nextIndex()).x) {
										d.setLocation(current);
										d.setTexture(Textures.DOOR);
										break;
									} else if (paths.get(
											it.previousIndex() - 1).y ==
											   paths.get(it.nextIndex()).y) {
										d.setLocation(current);
	 ...
	 * @param x coordinate
	 * @param y coordinate
	 * @return the Tile located in the 'tiles[][]' at x, y and null if there
	 * is none
	 */
	...
	/**
	 * @param startroom
	 * @return an EndRoom with a new StairDown instance as it's middle Tile
	 * content
	 */
	...
	/**
	 * @return a StartRoom with a new Character instance as it's middle Tile
	 * content
	 */
	 ...
	 * another thread to allow the program to do certain things in the
	 * meantime.
	 * Adds a two dimensional array of floats each at length SIZE to the
	 * BlockedQueue provided by DungeonGenerator to be processed in the main
	 * Thread
	 *
	 * @author Florian M. Becker
	 */
	private static class PerlinGeneration extends Thread {
		...
		public void run() {
			...
			for (int i = 0; i < values.length; i++) {
				for (int j = 0; j < values[i].length; j++) {
					values[i][j] = (float) ((MathUtils.perlinNoise(i * 0.075,
																   j * 0.075,
																   perlinSeedZ) +
											 1) * 0.5);
				}
